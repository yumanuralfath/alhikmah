use dioxus::prelude::*;
use hikmah_core::android::library_android::LibraryAndroid;
use hikmah_core::models::FileEntry;

#[component]
pub fn FileBrowser(library: Signal<LibraryAndroid>) -> Element {
...
#[component]
fn BrowserHeader(library: Signal<LibraryAndroid>) -> Element {
    let lib = library.read();
...
#[component]
fn FileItem(entry: FileEntry, index: usize, library: Signal<LibraryAndroid>) -> Element {
    rsx! {
        div {
            key: "{entry.name}",
...
#[component]
fn ErrorBanner(error: String, library: Signal<LibraryAndroid>) -> Element {
    rsx! {
